...
// Model ========================================
	internals.UserSchema = new Schema({
		...
		rating: { type: Number },
		active: { type: Date },
		login: { type: String, default: Date.now },
		date_created: { type: Date, default: Date.now }
	});
	internals.encryptPass = (pswd) => {

		let salt = Crypto.randomBytes(14).toString('hex');
		let sha1 = Crypto.createHash('sha1').update(pswd).digest('hex') + salt;
		let password = Crypto.createHash('sha256').update(sha1).digest('hex');
		let result = {
			salt,
			password
		};
