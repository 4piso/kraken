...
class UserCTRL extends BaseCTRL {
    ...
    login(request, reply) {
        ...
        if ('facebook' in payload) {
            query['facebook.token'] = payload.facebook.token;
        }

        // TODO: check como funca login with fb
        ...
        self.Collection.findOne(query, (err, user) => {
            ...
            // TODO: Check if jwt exists, and is valid
            if ('jwt' in _user) {
                return reply(JSend.success(_user)); // HTTP 201
            }
            ...
            console.log(_user.jwt);
            _user.save((err, resultUser) => {

                if (!err) {
                    return reply(JSend.success(resultUser)); // HTTP 201
                }
